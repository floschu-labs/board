...
/**
 * Check if value is an integer (rejects NaN, Infinity and non-integer reals;
 * the `position` columns are declared INTEGER in the DB)
 */
...
export function validateBoardData(data) {
  ...
  if (
    data.projects.length > MAX_ITEMS ||
    data.lists.length > MAX_ITEMS ||
    data.cards.length > MAX_ITEMS
  ) {
